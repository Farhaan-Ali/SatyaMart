...
export default function Users() {
  ...
  const fetchUsers = async () => {
    ...
    setErrorMsg(null);
    try {
      // Get all user roles first (superadmins can see all roles)
      ...
      const { data: supplierProfiles, error: supplierError } = await supabase
        ...
        .select('user_id, business_name, business_type, full_name');
      ...
      // Combine data based on user roles
      const combinedUsers = userRoles.map(role => {
        const supplierProfile = supplierProfiles?.find(p => p.user_id === role.user_id);
        const vendorProfile = vendorProfiles?.find(p => p.user_id === role.user_id);
        
        // Use profile email or generate placeholder
        const email = supplierProfile?.full_name || vendorProfile?.full_name || `user-${role.user_id.slice(0, 8)}`;
